...
> R: Inicializa el contador del carrito en 0. Es fundamental para evitar valores basura en memoria y garantizar el estado inicial.
...
> R: Falla. Esperaba 700 y obtuvo 350.
...
> R: Está en la línea 24 y el código dice total += c->items[i].precio;

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Deberia multiplicar por la cantidad del producto.
...
> R: Falla. el 5to carrito_agregar devuelve 1 y deberia devolver 0.
...
> R: No hay ninguna linea con ####, todas fueron cubiertas por los test unitarios y de integracion.
...
> R: Un test unitario evalua el funcionamiento de una funcion o componente de forma aislada, mientras que uno de integracion verifica como interactuan varias funciones o componentes entre si. El bug de carrito_total fue detectado primero por el test unitario test_total_con_cantidad.
...
> R: En C no hay verificacion de limites en tiempo de ejecucion, por lo que escribir fuera del array puede sobrescribir memoria, corromper datos o causar fallos de seguridad y segmentacion. En Python o Java el entorno verifica los indices y lanza una excepcion limpia, impidiendo acceso indebido a memoria.
...
> R: Permite detectar fallos de manera automatizada, objetiva y repetible. A simple vista el código puede parecer correcto, pero los tests evalúan comportamientos reales ante diferentes escenarios (como casos borde) y quedan guardados para evitar regresiones cuando el código cambie en el futuro.
...
> R: Porque cuando la cantidad es 1, el producto de precio * 1 da conceptualmente el mismo valor numérico que sólo precio. Por ende, la falta de la multiplicación en el código no alteró el resultado en ese caso particular.
